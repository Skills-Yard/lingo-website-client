...
export function SelectionCard({ title, icon, selected, onClick, className = "" }: SelectionCardProps) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center p-6 rounded-3xl border-2 border-b-4 transition-all duration-150 w-full min-h-35 shadow-md cursor-pointer active:translate-y-0.5 active:border-b-2 ${
        selected 
...
export function LevelCard({ codeSnippet, level, description, selected, onClick }: LevelCardProps) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center p-5 rounded-3xl border-2 border-b-4 transition-all duration-150 w-full min-h-37.5 shadow-md cursor-pointer active:translate-y-0.5 active:border-b-2 ${
        selected 
          ? "border-[#7c3aed] bg-[#f3e8ff] text-[#5b21b6] border-b-[6px] shadow-[0_0_14px_rgba(124,58,237,0.2)]" 
          : "border-slate-200 bg-white/80 hover:bg-slate-50/90 hover:border-slate-300 text-slate-700"
      }`}
    >
      <div className="font-mono text-xs md:text-sm text-indigo-600 dark:text-indigo-400 mb-3 min-h-8.75 flex items-center justify-center">
        {codeSnippet}
      </div>
